# -*- coding: utf-8 -*-
import utils
#import w2v.py # pretrain the w2v model
...
"""
Preprocess include: 
1. make embedding matrix include PAD and UNK
2. Create id2word and word2id 
3. Padding for ensuring the sentence lengths are equivalent
4. Turns label to tensor
...
class Preprocess():
    def __init__(self, sentences, sen_len, w2v_path="./w2v.model"):
        self.w2v_path = w2v_path# address of trained model 
        self.sentences = sentences
        self.sen_len = sen_len
        self.idx2word = [] # int -> str
        self.word2idx = {} # str -> int dictionary
        self.embedding_matrix = [] # E that presented in graph or NN that transform word to vec
    def get_w2v_model(self):
        # 把之前訓練好的 word to vec 模型讀進來
        self.embedding = Word2Vec.load(self.w2v_path)
        self.embedding_dim = self.embedding.vector_size # the vector size of each vocab
    def add_embedding(self, word):
        # 把 word 加進 embedding，並賦予他一個隨機生成的 representation vector
        # word 只會是 "<PAD>" 或 "<UNK>"
        vector = torch.empty(1, self.embedding_dim) #claim space for a new vector
        torch.nn.init.uniform_(vector) # initiate it randomly
        self.word2idx[word] = len(self.word2idx) # add the word to the end of dict with idx
        self.idx2word.append(word) # assign new idx to new word
        self.embedding_matrix = torch.cat([self.embedding_matrix, vector], 0)
        # for each word -> one-hot encoded -> add new word as one hot and -> vector
    
    def make_embedding(self, load=True):
        # This create new embedding on the base of trained embedding
        print("Get embedding ...")
    ...
    def pad_sequence(self, sentence):
        # 將每個句子變成一樣的長度
        #make each sentence to same length
    ...
    def sentence_word2idx(self):
        # 把句子裡面的字轉成相對應的 index
        #use the self.embedding matrix to transfer sentence to word
        sentence_list = []
        for i, sen in enumerate(self.sentences):
            ...
            for word in sen:
                if (word in self.word2idx.keys()):
                    ...
                    # in case that vocab not in our dictionary => UNK
...
class LSTM_Net(nn.Module):
    def __init__(self, embedding, embedding_dim, hidden_dim, num_layers, dropout=0.5, fix_embedding=True):
        #embedding will be the embedding we get from preprocessing.make_embedding
        super(LSTM_Net, self).__init__()
        # 製作 embedding layer
        # embedding here is the "embedding matrix" that we trained 
        # Add it to the model
...
def training(batch_size, n_epoch, lr, model_dir, train, valid, model, device):
    ...
    for epoch in range(n_epoch):
        ...
        for i, (inputs, labels) in enumerate(train):
            ...
            correct = utils.evaluation(outputs, labels) # 計算此時模型的 training accuracy
        ...
        with torch.no_grad():
            ...
            for i, (inputs, labels) in enumerate(valid):
                ...
                correct = utils.evaluation(outputs, labels) # 計算此時模型的 validation accuracy
                total_acc += (correct / batch_size)
                total_loss += loss.item()
